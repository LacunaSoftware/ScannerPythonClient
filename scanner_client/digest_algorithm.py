...
class DigestAlgorithm(object):
    ...
    def _eq_(self, instance):
        if instance is None:
            return False

        return self._oid == instance.oid
    ...
    def get_instance_by_api_model(algorithm):
        if algorithm == DigestAlgorithms.MD5.value:
            return DigestAlgorithm.MD5
        elif algorithm == DigestAlgorithms.SHA1.value:
            return DigestAlgorithm.SHA1
        elif algorithm == DigestAlgorithms.SHA256.value:
            return DigestAlgorithm.SHA256
        elif algorithm == DigestAlgorithms.SHA384.value:
            return DigestAlgorithm.SHA384
        elif algorithm == DigestAlgorithms.SHA512.value:
            return DigestAlgorithm.SHA512
        elif algorithm == DigestAlgorithms.SHA3_256.value:
    ...
    def check_length(self, digest_value):
        if len(digest_value) != self._byte_length:
            raise Exception('A %s digest should contain %s bytes, but a value '
                            'with %s bytes was given' % (self._name,
                                                         self._byte_length,
                                                         len(digest_value)))
